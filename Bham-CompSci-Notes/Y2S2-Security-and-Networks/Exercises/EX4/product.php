<?php
  ...
  if(isset($_REQUEST['quantity']) && is_numeric($_REQUEST['quantity'])) {
    ...
    $sql = "insert into basket (user_id, product_id, quantity) values('" . 
      ...
      mysqli_real_escape_string($db, $_REQUEST['quantity']) . ") on duplicate key update quantity = quantity + " .
      mysqli_real_escape_string($db, $_REQUEST['quantity']) . ";";
  ...
  if(isset($_GET['id'])) {
    ...
    $sql .= " AND p.id=".mysqli_real_escape_string($db, $_GET['id']);
  // are we searching?
